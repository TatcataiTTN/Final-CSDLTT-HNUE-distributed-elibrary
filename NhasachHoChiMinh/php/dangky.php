...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    // Debug logging
    error_log("HoChiMinh Registration: POST received");

    // Lấy dữ liệu từ form, loại bỏ khoảng trắng 2 đầu
    $username   = trim($_POST['username'] ?? '');
    $passwordRaw = $_POST['password'] ?? '';

    error_log("HoChiMinh Registration: username=$username");
    ...
    if ($username === '' || $passwordRaw === '') {
        ...
        error_log("HoChiMinh Registration: Validation failed - empty fields");
    } else {
        // Mã hóa mật khẩu
        ...
        if ($existingUser) {
            ...
            error_log("HoChiMinh Registration: User already exists");
        } else {
            // Thêm user mới
            try {
                ...
                error_log("HoChiMinh Registration: Insert count = $insertCount");

                if ($insertCount > 0) {
                    $message = "Đăng ký thành công! <a href='dangnhap.php'>Đăng nhập ngay</a>";
                    error_log("HoChiMinh Registration: SUCCESS - $username");
                } else {
                    $message = "Lỗi khi thêm user.";
                    error_log("HoChiMinh Registration: Insert failed - count is 0");
                }
            } catch (Exception $e) {
                $message = "Lỗi khi thêm user: " . $e->getMessage();
                error_log("HoChiMinh Registration: Exception - " . $e->getMessage());
            }
        }
    }

    error_log("HoChiMinh Registration: Final message = $message");
}
?>
<!DOCTYPE html>
<html lang="vi">
