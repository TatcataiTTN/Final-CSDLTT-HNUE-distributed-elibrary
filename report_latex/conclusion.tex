...
\begin{enumerate}
    \item \textbf{Xây dựng thành công kiến trúc phân tán 4 node}:
    \begin{itemize}
        \item Central Hub (Nhasach) - Trung tâm điều phối và quản lý dữ liệu tổng
        \item 3 chi nhánh vùng: NhasachHaNoi, NhasachDaNang, NhasachHoChiMinh
        \item Mỗi node hoạt động độc lập với database riêng biệt
        \item Đồng bộ dữ liệu qua REST API
    \end{itemize}

    \item \textbf{Triển khai MongoDB Replica Set với Docker}:
    \begin{itemize}
        \item 3 MongoDB instances: mongo1:27017, mongo2:27018, mongo3:27019
        \item Replica Set tên ``rs0'' với automatic failover
        \item Read Preference: primaryPreferred cho cân bằng tải đọc
        \item Write Concern: majority đảm bảo consistency
    \end{itemize}

    \item \textbf{Dữ liệu thực tế đã tích hợp}:
    \begin{itemize}
        \item Tổng cộng 909 đầu sách (Central: 509, HN: 162, DN: 127, HCM: 111)
        \item 38 người dùng đã đăng ký (13 HN, 12 DN, 11 HCM, 2 Central)
        \item 46 đơn mượn sách đã xử lý
        \item 10 thể loại sách khác nhau
    \end{itemize}
\end{enumerate}
...
\begin{enumerate}
    \item \textbf{Hệ thống quản lý sách hoàn chỉnh}:
    \begin{itemize}
        \item CRUD operations với validation đầy đủ
        \item Tìm kiếm Full-text với TEXT index hỗ trợ tiếng Việt
        \item Phân loại theo thể loại và chi nhánh
        \item Quản lý số lượng và giá thuê
    \end{itemize}

    \item \textbf{Hệ thống người dùng và phân quyền}:
    \begin{itemize}
        \item Role-Based Access Control: admin và customer
        \item JWT Authentication với expiration 24 giờ
        \item bcrypt password hashing (cost factor 12)
        \item Quản lý số dư tài khoản
    \end{itemize}

    \item \textbf{Quy trình mượn/trả sách}:
    \begin{itemize}
        \item Giỏ hàng với session management
        \item Thanh toán và trừ số dư tự động
        \item Lịch sử mượn sách chi tiết
        \item Thống kê theo thời gian thực
    \end{itemize}

    \item \textbf{Dashboard thống kê với 6 biểu đồ Chart.js}:
    \begin{itemize}
        \item Tổng quan: sách, users, orders, doanh thu
        \item Biểu đồ tròn: phân bố theo thể loại
        \item Biểu đồ cột: số sách theo chi nhánh
        \item Biểu đồ đường: trend mượn sách theo tháng
    \end{itemize}
\end{enumerate}
...
\begin{table}[H]
\centering
\caption{Tổng hợp kết quả benchmark}
\begin{tabular}{|l|r|r|}
\hline
