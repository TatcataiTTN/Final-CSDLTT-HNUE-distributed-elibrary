...
MongoDB Replica Set hoạt động theo mô hình Master-Slave với cơ chế bầu cử tự động. Node \textbf{Primary} (mongo1) nhận tất cả các thao tác ghi (insert, update, delete), sau đó đồng bộ dữ liệu sang các node \textbf{Secondary} thông qua oplog (operation log). Các Secondary gửi heartbeat mỗi 2 giây để kiểm tra trạng thái Primary. Chiến lược truy vấn: \textit{Write to Primary, Read from Primary hoặc Secondary} - giúp phân tải đọc và tăng throughput hệ thống.
